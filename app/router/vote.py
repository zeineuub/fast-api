...
def vote(vote: schema.Vote, db:Session = Depends(get_db), current_user:int= Depends(oauth2.get_current_user)):
    ...
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail=f"the post {vote.post_id} was not found")
    nb_vote = db.query(models.Vote).filter(models.Vote.user_id== current_user.id).count()
    ...
    if(vote.dir ==1):
        ...
        db.add(new_vote)
        db.commit()
        return {"message": f"successfully added vote"}
    else:
        if not found_vote:
